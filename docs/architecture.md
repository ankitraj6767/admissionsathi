...
| AI provider | `src/services/ai.service.ts` | `ProviderAdapter { id, model, complete(ProviderRequest), stream?(ProviderRequest) }` | `mockAdapter` (extractive, no invented facts), streaming NVIDIA NIM adapter, `openaiAdapter`, `anthropicAdapter` | `AI_PROVIDER` via `adapterFor(provider)`; falls back to `mockAdapter` on error, missing key or missing website context |
